...
cars=['Hyundai Santro Xing', 'Mahindra Jeep CL550', 'Hyundai Grand i10',
...
with col1:
    company= st.selectbox('Select the company',sorted(companies))

with col2:
    car=st.selectbox('Select the car', sorted(cars))
...
if st.button('Predict Price'):
    inputs=pd.DataFrame({'name':[car], 'company':[company],'year':[year],'kms_driven':[kms_driven],'fuel_type':[fuel_type]})
    
    result=pipe.predict(inputs)
    predicted_price=round(result[0])
    st.text("Predicted Price = "+ 'Rs '+ str(predicted_price))
